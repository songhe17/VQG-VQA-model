...
def eval(image_features,question):
    ...
    for label in reversed(y_sort_index[-1]):
        a = str(round(y_output[0,label]*100,2)).zfill(5), labelencoder.inverse_transform(label)
    return float(a[0])

def main():
    data_path='./data/prepro.pkl'
    with open(data_path,'rb') as f:
        data = pickle.load(f)
    # image_feats = [x['feats'] for x in data]
    # questions = [x['questions'] for x in data]
    ans_confi = []
    for i in range(len(data)):
        question = data[i]['question']
        feats = data[i]['feats']
        confi = eval(feats,question)
        data[i].update({'confi':confi})
